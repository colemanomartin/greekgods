...
class river_a_fast(Mode):
    def mode_start(self, **kwargs):
        self.log.info('******************** registering mode_a handler')
        self.add_mode_event_handler('accelerate_a', self.enable_a)
        self.add_mode_event_handler('normalize_a', self.deactivate_a)

# Note accelerate_a cannot be called imeadiately when mode starts, it seems to take time for the mode handler to register it
    def enable_a(self, **kwargs):
        self.log.info('******************** sending Jans_a rules')
    ...
    def deactivate_a(self, **kwargs):
        self.log.info('******************** clearing Jans_a rules')
        enable_switch = self.machine.switches['s_u_left_ear_3']
        disable_switch = self.machine.switches['s_u_left_ear_2']
        coil = self.machine.coils['c_u_left_magnet']
