...
class river_b_fast(Mode):
    def mode_start(self, **kwargs):
        self.log.info('******************** registering mode_b handler')
        self.add_mode_event_handler('accelerate_b', self.enable_b)
        self.add_mode_event_handler('normalize_b', self.deactivate_b)

# Note accelerate_b cannot be called imeadiately when mode starts, it seems to take time for the mode handler to register it
    def enable_b(self, **kwargs):
        self.log.info('******************** sending Jans_b rules')
        enable_switch = self.machine.switches['s_u_left_ear_0']
        disable_switch = self.machine.switches['s_u_left_ear_1']
    ...
    def deactivate_b(self, **kwargs):
        self.log.info('******************** clearing Jans_b rules')
        enable_switch = self.machine.switches['s_u_left_ear_0']
        disable_switch = self.machine.switches['s_u_left_ear_1']
        coil = self.machine.coils['c_u_left_magnet']
