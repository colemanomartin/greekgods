...
class river_c_fast(Mode):
    def mode_start(self, **kwargs):
        self.log.info('******************** registering mode_c handler')
        self.add_mode_event_handler('accelerate_c', self.enable_c)
        self.add_mode_event_handler('normalize_c', self.deactivate_c)

# Note accelerate_c cannot be called imeadiately when mode starts, it seems to take time for the mode handler to register it
    def enable_c(self, **kwargs):
        self.log.info('******************** sending Jans_c rules')
        enable_switch = self.machine.switches['s_u_right_ear_3']
        disable_switch = self.machine.switches['s_u_right_ear_2']
    ...
    def deactivate_c(self, **kwargs):
        self.log.info('******************** clearing Jans_c rules')
        enable_switch = self.machine.switches['s_u_right_ear_3']
        disable_switch = self.machine.switches['s_u_right_ear_2']
        coil = self.machine.coils['c_u_right_magnet']
